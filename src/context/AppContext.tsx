...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [toasts, setToasts] = useState<{ id: number; message: string; type: 'success' | 'error' | 'info' }[]>([]); const [products, setProducts] = useState<any[]>([]);
  ...
  useEffect(() => { if (cart.length === 0) return; const timer = setTimeout(() => { const currentUser = auth.currentUser; if (currentUser?.email) { cartApi.reportAbandoned({ email: currentUser.email, displayName: currentUser.displayName || 'Customer', total: cartTotal, cartItems: cart, currency }).catch(err => console.error('Abandoned cart report failed:', err)); reportActivity('abandoned_cart', { itemCount: cart.length, total: cartTotal, currency }); } }, 30 * 60 * 1000); return () => clearTimeout(timer); }, [cart, cartTotal, currency]);
  const addToast = (message: string, type: 'success' | 'error' | 'info' = 'success') => { const id = Date.now(); setToasts(prev => [...prev, { id, message, type }]); window.setTimeout(() => setToasts(prev => prev.filter(t => t.id !== id)), 4000); };
  return <AppContext.Provider value={{ currency, setCurrency, formatPrice, wishlist, toggleWishlist, searchQuery, setSearchQuery, isSearchOpen, setIsSearchOpen, cart, addToCart, removeFromCart, updateQuantity, clearCart, cartTotal, addToast, reportActivity, products }}>{children}<div className="fixed bottom-8 right-8 z-[200] flex flex-col gap-4 pointer-events-none">{toasts.map(toast => <div key={toast.id} className={`pointer-events-auto min-w-[300px] p-4 rounded-lg shadow-2xl border-l-4 flex flex-col relative overflow-hidden animate-slide-up ${toast.type === 'success' ? 'bg-white border-green-500' : toast.type === 'error' ? 'bg-white border-red-500' : 'bg-white border-brand-gold'}`}><div className="flex items-center justify-between mb-1"><span className={`text-[10px] font-bold tracking-widest uppercase ${toast.type === 'success' ? 'text-green-600' : toast.type === 'error' ? 'text-red-600' : 'text-brand-gold'}`}>{toast.type === 'success' ? 'Success' : toast.type === 'error' ? 'Error' : 'Information'}</span><button onClick={() => setToasts(prev => prev.filter(t => t.id !== toast.id))} className="text-gray-400 hover:text-brand-dark">×</button></div><p className="text-xs font-medium text-brand-dark">{toast.message}</p></div>)}</div></AppContext.Provider>;
};
