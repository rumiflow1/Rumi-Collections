...
import { BRAND, normalizeBrandText } from '../config/brand';
...
const defaultGlobalConfig: Config = {
  elements: {},
  branding: { brandName: BRAND.name, name: BRAND.name },
  ...
  header: { isVisible: true, isCentered: false, logoText: BRAND.name, logoImage: '', logoColor: '#D4AF37', logoSize: '22px', logoWidth: '3rem', logoHeight: '3rem', logoPadding: '0px', logoFontFamily: 'Playfair Display', navLinks: [
  ...
  footer: { isVisible: true, brandName: BRAND.name, description: 'Elevating your style with premium fabrics and timeless designs.', copyright: `© 2026 ${BRAND.name}. All rights reserved.`, shopLinks: [], supportLinks: [], socials: [] },
  aiConcierge: { isEnabled: true, brandVoice: 'Sophisticated, confident, and professional', systemInstruction: `You are an AI shopping assistant for ${BRAND.name}. Be transparent that you are an AI assistant, and help users find products and understand the store.`, model: 'gemini-3.5-flash', welcomeMessage: `Greetings. I am your AI stylist for ${BRAND.name}. How may I assist you today?` },
...
const replaceBrandDeep = (value: any): any => {
  if (typeof value === 'string') return normalizeBrandText(value);
  if (Array.isArray(value)) return value.map(replaceBrandDeep);
  if (value && typeof value === 'object') return Object.fromEntries(Object.entries(value).map(([k, v]) => [k, replaceBrandDeep(v)]));
  return value;
};
...
const sanitizeConfig = (config: any): Config => {
  ...
  const elements = Object.fromEntries(Object.entries(merged.elements || {}).map(([key, raw]: any) => [key, {
    ...(raw || {}), content: asText(raw?.content ?? raw?.text, ''), color: asText(raw?.color, ''), fontFamily: asText(raw?.fontFamily, ''),
    fontSize: asText(raw?.fontSize, ''), link: asText(raw?.link, ''), background: asText(raw?.background, ''), isVisible: raw?.isVisible !== false,
  }]));
  return { ...merged, branding: { ...(merged.branding || {}), brandName: BRAND.name, name: BRAND.name }, elements,
    header: { ...(merged.header || {}), logoText: BRAND.name }, footer: { ...(merged.footer || {}), brandName: BRAND.name, description: asText(merged.footer?.description, ''), copyright: `© 2026 ${BRAND.name}. All rights reserved.` },
    aiConcierge: { ...(merged.aiConcierge || {}), model: 'gemini-3.5-flash', systemInstruction: `You are an AI shopping assistant for ${BRAND.name}. Be transparent that you are an AI assistant, concise, helpful and practical.` }
  } as Config;
};

export function ConfigProvider({ children }: { children: React.ReactNode }) {
  const [SiteConfig, setSiteConfig] = useState<Config>(() => {
    try { const cached = localStorage.getItem('luxe_site_config'); return cached ? sanitizeConfig(JSON.parse(cached)) : sanitizeConfig(defaultGlobalConfig); }
    catch { return sanitizeConfig(defaultGlobalConfig); }
  });
  const [loading, setLoading] = useState(true);
  const fetchConfig = async () => {
    try {
      const response = await axios.get('/api/config');
      if (response.data) { const merged = sanitizeConfig(response.data); setSiteConfig(merged); localStorage.setItem('luxe_site_config', JSON.stringify(merged)); }
