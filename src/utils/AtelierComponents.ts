...
export const renderButton = (text: string, path: string, color = '#0A0A0A') => `
  <div style="text-align:center;margin:30px 0;">
    <a href="${ATELIER_CONFIG.frontendUrl}${path}" style="background:${color};color:#fff !important;padding:15px 34px;text-decoration:none;font-size:11px;font-weight:700;letter-spacing:.16em;text-transform:uppercase;border-radius:2px;display:inline-block;">${escapeHtml(text)}</a>
  </div>
`;
...
export const renderTrendingGrid = (products: any[] = []) => {
  ...
  live.forEach((p: any) => {
    ...
    const priceText = Number.isFinite(price) ? `${price.toLocaleString()} ${escapeHtml(p.currency || 'PKR')}` : '';
    if (!id || !image) return;
