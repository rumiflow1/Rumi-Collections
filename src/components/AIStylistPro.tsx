...
export default function AIStylistPro() {
  ...
  const quickSuggestions = useMemo(() => {
    const ranked = [...liveProducts].sort((a: any, b: any) => Number(Boolean(b.isFeatured || b.isNewArrival)) - Number(Boolean(a.isFeatured || a.isNewArrival)));
    const productSuggestions = ranked.slice(0, 4).map((p: any) => `Show ${p.name || p.title}`);
    const siteActions = [
      ...(ranked.some((p: any) => p.isNewArrival) ? ['New arrivals'] : []),
      ...(ranked.some((p: any) => p.isFeatured) ? ['Featured collection'] : []),
      'Help me choose'
    ];
    return [...new Set([...productSuggestions, ...siteActions])].slice(0, 5);
  }, [liveProducts]);

  useEffect(() => {
    if (messages.length === 0 && SiteConfig) {
      setMessages([{ role: 'ai', text: `Hello. I’m your ${String(SiteConfig.header?.logoText || 'DENFIT')} shopping assistant. I can answer from the current collection and store information, and I can open matching products or site links for you.` }]);
    }
  ...
  const productHaystack = (p: any) => [p.name, p.title, p.category, p.collectionName, p.description, ...(Array.isArray(p.colors) ? p.colors : []), ...(Array.isArray(p.tags) ? p.tags : [])].filter(Boolean).join(' ').toLowerCase();
  ...
  const getSocial = (platform: string) => {
    const all = [
      ...(Array.isArray(SiteConfig?.footer?.socials) ? SiteConfig.footer.socials : []),
      ...(Array.isArray(SiteConfig?.footer?.socialIcons) ? SiteConfig.footer.socialIcons : []),
      ...(Array.isArray(SiteConfig?.announcementBar?.socials) ? SiteConfig.announcementBar.socials : []),
      ...(Array.isArray(SiteConfig?.announcementBar?.socialIcons) ? SiteConfig.announcementBar.socialIcons : [])
    ];
    return all.find((s: any) => String(s.platform || s.icon || '').toLowerCase().replace(/\s/g, '') === platform.toLowerCase().replace(/\s/g, '') && (s.url || s.link));
  };
  const socialUrl = (social: any) => social?.url || social?.link || '';
  const extractWhatsAppNumber = (url: string) => { try { return decodeURIComponent(url).match(/(?:wa\.me\/|phone=)([+\d]+)/i)?.[1] || ''; } catch { return ''; } };

  const processNavigation = (text: string, alreadyNavigated = false) => {
    ...
    while ((match = regex.exec(text))) {
      ...
      if (kind === 'NAV') {
        ...
        navigated = true;
      } else if (kind === 'EXT' && param && /^https?:\/\//i.test(param)) { window.open(param, '_blank', 'noopener,noreferrer'); navigated = true; }
    }
    return navigated;
  };
  ...
  const handleSend = async (preset?: string) => {
    ...
    if (/\bwhats?app\b/i.test(q)) { const social = getSocial('WhatsApp'); const url = socialUrl(social); if (url) { window.open(url, '_blank', 'noopener,noreferrer'); immediateNavigation = true; } }
    else if (/\btik\s*tok\b/i.test(q)) { const social = getSocial('TikTok'); const url = socialUrl(social); if (url) { window.open(url, '_blank', 'noopener,noreferrer'); immediateNavigation = true; } }
    else if (/\binstagram\b/i.test(q)) { const social = getSocial('Instagram'); const url = socialUrl(social); if (url) { window.open(url, '_blank', 'noopener,noreferrer'); immediateNavigation = true; } }
    ...
    try {
      ...
      // The backend re-reads MongoDB for every request. This browser payload is only a small fallback/context hint.
      const compactProducts = liveProducts.slice(0, 30).map((p: any) => ({ id: p.id || p._id, name: p.name || p.title, price: Number(p.price || 0), currency: p.currency || 'PKR', image: p.image || p.images?.[0] || '', category: p.category || '', collectionName: p.collectionName || '', description: String(p.description || '').slice(0, 240), colors: Array.isArray(p.colors) ? p.colors.slice(0, 8) : [], sizes: Array.isArray(p.sizes) ? p.sizes.slice(0, 8) : [], stock: Number(p.stock ?? 0), isNewArrival: Boolean(p.isNewArrival), isFeatured: Boolean(p.isFeatured) }));
      const response = await axios.post('/api/ai/stylist', { message: userMessage, history, products: compactProducts, siteConfig: { header: SiteConfig?.header, footer: SiteConfig?.footer, announcementBar: SiteConfig?.announcementBar, pages: { faq: SiteConfig?.pages?.faq, privacyPolicy: SiteConfig?.pages?.privacyPolicy, returnPolicy: SiteConfig?.pages?.returnPolicy, shippingPolicy: SiteConfig?.pages?.shippingPolicy, termsOfService: SiteConfig?.pages?.termsOfService } } }, { timeout: 8000 });
      const aiText = String(response.data?.text || 'I could not find enough current store information to answer that.');
      processNavigation(aiText, immediateNavigation);
      const cleanText = aiText.replace(/\[(?:NAV|EXT):[^\]]+\]/gi, '').trim();
      let displayText = cleanText;
      if (/\bwhats?app\b/i.test(userMessage)) { const social = getSocial('WhatsApp'); const number = socialUrl(social) ? extractWhatsAppNumber(socialUrl(social)) : ''; if (number && !displayText.includes(number)) displayText = `${displayText}\n\nWhatsApp: ${number}`; }
      ...
      setMessages(prev => [...prev, { role: 'ai', text: 'The live assistant is taking a moment. Please try again.' }]);
  ...
  return <>
    ...
    <AnimatePresence>{isOpen && <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="fixed bottom-20 right-4 md:right-8 z-[100] w-[calc(100%-2rem)] md:w-96 h-[500px] max-h-[70vh] bg-white shadow-2xl flex flex-col overflow-hidden border border-brand-dark/5 rounded-xl">
      ...
      <div className="flex-grow overflow-y-auto p-4 space-y-4 bg-brand-cream/30">{messages.map((msg, idx) => <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[86%] p-3 text-sm whitespace-pre-line ${msg.role === 'user' ? 'bg-brand-dark text-white rounded-l-lg rounded-tr-lg' : 'bg-white text-brand-dark border border-brand-dark/5 rounded-r-lg rounded-tl-lg'}`}>{msg.text}</div></div>)}{isLoading && <div className="flex justify-start"><div className="bg-white p-3 rounded-r-lg rounded-tl-lg flex items-center gap-2 text-xs text-gray-500"><Loader2 size={18} className="animate-spin text-brand-gold" /> Checking the live store…</div></div>}<div ref={messagesEndRef} /></div>
