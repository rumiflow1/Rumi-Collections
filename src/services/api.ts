...
const readCachedProducts = (): any[] => {
  try { const raw = localStorage.getItem(PRODUCT_CACHE_KEY); const parsed = raw ? JSON.parse(raw) : []; return Array.isArray(parsed) ? parsed.map(normalizeProduct) : []; } catch { return []; }
};
const writeCachedProducts = (products: any[]) => { try { localStorage.setItem(PRODUCT_CACHE_KEY, JSON.stringify(products.map(normalizeProduct))); } catch { /* cache is optional */ } };

const api = axios.create({ baseURL: API_URL, timeout: 12000, headers: { 'Content-Type': 'application/json' } });
api.interceptors.request.use((config) => { if (config.url?.includes('/admin/upload') && typeof FormData !== 'undefined' && config.data instanceof FormData) { const image = config.data.get('image'); if (image && !config.data.get('file')) config.data.append('file', image); } return config; });
api.interceptors.response.use((response) => { if (response.config.url?.endsWith('/config')) response.data = mergeConfig(response.data); if (response.config.url?.includes('/admin/upload') && response.data?.url && !response.data.imageUrl) response.data.imageUrl = response.data.url; if (response.config.url?.endsWith('/admin/customers') && Array.isArray(response.data)) response.data = { users: response.data, logs: [] }; return response; });
...
export const discountApi = { verify: (code: string, orderAmount = 0) => api.post('/discounts/verify', { code: code.trim().toUpperCase(), orderAmount }) };
...
export const productApi = {
  getAll: async () => {
    try {
      ...
      if (Array.isArray(raw)) { const normalized = raw.map(normalizeProduct); writeCachedProducts(normalized); return { ...response, data: normalized }; }
      return { ...response, data: [] };
    } catch (error) {
      const cached = readCachedProducts();
      if (cached.length) return { data: cached, status: 200, statusText: 'Cached', headers: {}, config: {} } as any;
      throw error;
    }
  },
  getById: async (id: string) => {
    try { const response = await api.get(`/products/${encodeURIComponent(id)}`); return { ...response, data: normalizeProduct(response.data?.product || response.data) }; }
    catch (error) { const cached = readCachedProducts(); const match = cached.find((product: any) => String(product.id || product._id) === String(id)); if (match) return { data: normalizeProduct(match), status: 200, statusText: 'Cached', headers: {}, config: {} } as any; throw error; }
  },
};

export const cartApi = { reportAbandoned: (cartData: any) => api.post('/cart/abandoned', cartData) };

const compressImage = (file: File, maxBytes = 2 * 1024 * 1024): Promise<File> => new Promise((resolve) => { if (file.size <= maxBytes) return resolve(file); const img = new Image(); const url = URL.createObjectURL(file); img.onload = () => { URL.revokeObjectURL(url); const scale = Math.min(1, 1800 / Math.max(img.width, img.height)); const canvas = document.createElement('canvas'); canvas.width = Math.max(1, Math.round(img.width * scale)); canvas.height = Math.max(1, Math.round(img.height * scale)); const ctx = canvas.getContext('2d'); if (!ctx) return resolve(file); ctx.drawImage(img, 0, 0, canvas.width, canvas.height); canvas.toBlob((blob) => resolve(blob && blob.size < file.size ? new File([blob], file.name.replace(/\.[^.]+$/, '.jpg'), { type: 'image/jpeg' }) : file), 'image/jpeg', 0.82); }; img.onerror = () => { URL.revokeObjectURL(url); resolve(file); }; img.src = url; });
export const uploadApi = { uploadImage: async (file: File) => { const safeFile = await compressImage(file); const formData = new FormData(); formData.append('file', safeFile); const response = await api.post('/admin/upload', formData, { headers: { 'Content-Type': 'multipart/form-data' }, maxContentLength: 3 * 1024 * 1024, maxBodyLength: 3 * 1024 * 1024 }); return response.data; } };
export default api;
